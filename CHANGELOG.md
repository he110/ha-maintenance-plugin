...
### ✨ Новые возможности
- **Встроенные глобальные сенсоры подсчета** - автоматически создаются при установке интеграции
- `sensor.maintenance_overdue_count` - количество просроченных компонентов
- `sensor.maintenance_due_count` - количество компонентов требующих обслуживания
- **Богатые атрибуты сенсоров** - списки названий, entity_id и подробная информация о каждом компоненте
- **Автоматическое обновление** - сенсоры обновляются при изменении статуса любого компонента
- **Динамические иконки** - меняются в зависимости от количества компонентов

### 🔧 Улучшения
- Больше не нужно создавать template сенсоры вручную в configuration.yaml
- Готовые сенсоры для использования в Lovelace карточках и автоматизациях
- Оптимизированная производительность с подпиской на события изменения состояний

### 📱 Интеграция с UI
- Идеально подходят для использования с Bubble Card и другими кастомными карточками
- Готовые данные для создания красивых дашбордов обслуживания

## [1.2.3] - 2025-06-16

### ✨ Новые возможности
- **Service для установки даты последнего обслуживания** - `maintainable.set_last_maintenance`
- Возможность легко установить дату обслуживания из прошлого для компонентов, которые были обслужены до добавления в интеграцию

### 🔧 Улучшения
- Новый service принимает entity_id сенсора и дату обслуживания
- Автоматическая синхронизация между связанными сущностями при изменении даты
- Удобный интерфейс с селекторами для выбора сущности и даты

### 🔧 Исправлено
- Исправлены ошибки валидации services.yaml - убраны неподдерживаемые поля response

## [1.2.2] - 2025-06-16

### ✨ Новые возможности
- **Поддержка английского языка** - добавлена полная локализация en.json
- **Иконка для HACS** - добавлена иконка интеграции для отображения в HACS

### 📚 Документация
- Переписаны README.md, info.md, TROUBLESHOOTING.md на английский язык
- Убрана лишняя техническая информация для лучшего пользовательского опыта
- Упрощена документация с фокусом на конечных пользователей

## [1.2.1] - 2025-06-16

### 🔧 Исправлено
- **Services теперь возвращают только основные сенсоры** - убрано дублирование данных для трех сущностей одного компонента
- Services `get_overdue_items`, `get_due_items` и `get_all_items` теперь возвращают только сенсоры статуса (с окончанием `_status`)
- Это устраняет дублирование одинаковых данных для сенсора статуса, сенсора дней и кнопки

### 📊 Изменения в данных
- Вместо 3 записей на компонент теперь возвращается 1 запись с полной информацией
- Все необходимые данные (статус, дни, даты) по-прежнему доступны в основном сенсоре статуса
